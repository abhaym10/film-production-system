...
/api/projects/${projectId}/schedule`)
    ...
    const budgetData = budget.expenses.map((item, index) => ({
        name: item.title || `Expense ${index+1}`,
        amount: item.amount || 0
    }));

    const maxItem = budgetData.reduce((max, item) => (item.amount > (max?.amount || 0) ? item : max), null);

    const pieData = [
        { name: "Spent", value: budget.actual },
                ...
                <div style={{ ...cardStyle, marginBottom: "24px" }}>
                    ...
                    <ResponsiveContainer width="100%" height={300}>
                        <BarChart data={budgetData}>
                            <CartesianGrid strokeDasharray="33" stroke="#333" />
                            ...
                            <Bar dataKey="amount" radius={[6, 6 , 0, 0]} >
                                {budgetData.map((entry, index) => (
                                    <Cell
                                        key={index}
                                        fill={entry.amount > 500 ? "#e74c3c" : "#2ecc71"}
                                    />
                                ))}
                            </Bar>
                        </BarChart>
                    ...
                    <p style={{ color: "#aaa", fontSize: "14px", marginTop: "10px" }}>
                        Highest Expense: ₹{Math.max(...budgetData.map(i => i.amount || 0))}
                    </p>

                    <p style={{color: "#aaa", fontSize: "14px" }}>
                        Top Category: {maxItem?.name || "N/A"}
                    </p>
                    ...
                    <p style={{ fontSize: "14px", color: "#aaa", marginTop: "10px" }}>
                        ...
                        : budget.remaining < budget.planned * 0.2
